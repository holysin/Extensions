...
extension TypeWrapper where T == Timer {
    static func scheduledTimerWithTimeInterval(_ timeInterval: TimeInterval, repeats: Bool, callback: @escaping ((Timer) -> Void)) -> Timer {
    ...
    static func timerWithTimeInterval(_ timeInterval: TimeInterval, repeats: Bool, callback: @escaping ((Timer) -> Void)) -> Timer {
        let callbackWrapper = CallbackWrapper(callback)
